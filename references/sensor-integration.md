...
| ID | 패턴 | 트리거 키워드 | Line |
|----|------|-------------|------|
| I01 | 자이로 드리프트 미보상 | gyro, angle += gyro, drift | 52 |
| I02 | 가속도계 기반 각도의 진동 문제 | atan2, accel, pitch, vibration | 70 |
| I03 | 자기장 간섭 무시 | mag, heading, atan2, magnetometer | 91 |
| I04 | IMU 축 정렬 오류 | imu_data, pitch, roll, yaw, mount | 108 |
| F01 | 힘 센서 0점 드리프트 | read_adc, read_force, force_offset, tare | 130 |
| F02 | 중력 보상 없음 | force_z, wrench, gravity, compensation | 148 |
| F03 | 힘 센서 과부하 미감지 | read_force, load_cell, saturation, range | 166 |
| F04 | 6축 힘/토크 센서 크로스토크 | ch0, scale, calibration_matrix, crosstalk | 190 |
| E01 | 전기 노이즈로 인한 카운트 점프 | count, prev_count, encoder, noise | 212 |
| E02 | 인덱스 펄스 미활용 | encoder_isr, index_pulse, A_rising | 229 |
| SF01 | 상보 필터 시정수 부적절 | alpha, complementary, tau, filter | 259 |
| SF02 | 칼만 필터 프로세스 노이즈 미튜닝 | Q, R, np.eye, process_noise, kalman | 278 |
| SF03 | 센서 고장 시 퓨전 출력 신뢰 | kalman_update, fault, outlier, fusion | 298 |
| C01 | 캘리브레이션 데이터 만료 | load_calibration, timestamp, expire | 322 |
| C02 | 캘리브레이션 순서 오류 | calibrate, start_motors, sequence | 343 |
| C03 | 단일 조건 캘리브레이션 | bias, measure_bias, temperature, interpolate | 363 |
| D01 | 필터 초기화 미처리 | filtered_value, alpha, initialized | 388 |
| D02 | 이동 평균에 의한 위상 지연 무시 | moving_avg, WINDOW, phase, delay | 416 |
| D03 | ADC 분해능과 물리량 매핑 오류 | adc_raw, ADC_RESOLUTION, 4096, 1024 | 438 |
| D04 | 센서 데이터 타임스탬프 누락 | read_imu, read_force, timestamp | 456 |
| T01 | 멀티센서 샘플링 동기화 미처리 | read_imu, trigger_all, sync, DMA | 478 |
| T02 | 센서 I2C 주소 충돌 | i2c_read, 0x68, 0x69, address | 501 |
| T03 | SPI 클럭 과속 | BAUDRATEPRESCALER, SPI, clock | 521 |
