...
| ID | 패턴 | 카테고리 | Line |
|----|------|---------|------|
| P01 | delay() 기반 타이밍 | 타이밍 | 62 |
| P02 | 타이머 오버플로우 무시 | 타이밍 | 93 |
| P03 | 하드코딩된 타이밍 상수 | 타이밍 | 110 |
| P04 | Deadline miss 미감지 | 타이밍 | 127 |
| P05 | 주기적 태스크에서 가변 실행 경로 | 타이밍 | 161 |
| P06 | 부동소수점 연산의 타이밍 변동 | 타이밍 | 189 |
| P07 | Watchdog 미설정 | 타이밍 | 205 |
| P08 | 인터럽트 내 긴 처리 | 인터럽트 | 233 |
| P09 | 공유 변수 비원자 접근 | 인터럽트 | 258 |
| P10 | 인터럽트 우선순위 역전 | 인터럽트 | 283 |
| P11 | 인터럽트 비활성화 장시간 유지 | 인터럽트 | 303 |
| P12 | 인터럽트 재진입 미방지 | 인터럽트 | 324 |
| P13 | 스택 오버플로우 | 메모리 | 351 |
| P14 | 힙 단편화 | 메모리 | 375 |
| P15 | malloc 반환값 미확인 | 메모리 | 400 |
| P16 | DMA 버퍼 캐시 불일치 | 메모리 | 421 |
| P17 | 링 버퍼 경계 오류 | 메모리 | 440 |
| P18 | TOCTOU | 동기화 | 468 |
| P19 | 뮤텍스 없는 다중 태스크 접근 | 동기화 | 490 |
| P20 | 데드락 | 동기화 | 512 |
| P21 | 큐 오버플로우 미처리 | 동기화 | 533 |
| P22 | 정수 오버플로우 | 수치 | 554 |
| P23 | 부동소수점 비교 | 수치 | 571 |
| P24 | 나누기 0 미검사 | 수치 | 587 |
| P25 | 단위 혼동 | 수치 | 606 |
| P26 | 누적 오차 | 수치 | 622 |
| P27 | GPIO 초기 상태 미설정 | 하드웨어 | 644 |
| P28 | ADC 읽기 순서 오류 | 하드웨어 | 663 |
| P29 | SPI/I2C 버스 경합 | 하드웨어 | 688 |
| P30 | PWM 주파수/해상도 불일치 | 하드웨어 | 710 |
| P31 | 기본 케이스 누락 | 상태머신 | 734 |
| P32 | 상태 전이 검증 누락 | 상태머신 | 763 |
| P33 | 비상 정지 후 자동 재시작 | 상태머신 | 792 |
| P34 | Watchdog feed가 상태와 무관 | 상태머신 | 821 |
